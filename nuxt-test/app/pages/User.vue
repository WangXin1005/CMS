...
function onMailBlur(val) { if (!val) { mailError.value = ""; return; } mailError.value = validateEmail(val) || ""; }
function onPwdBlur(val) { if (!val) { pwdError.value = ""; return; } pwdError.value = validatePassword(val) || ""; }

async function loadData() {
  loading.value = true;
  try { const res = await getUserList(currentPage.value, pageSize.value); tableData.value = res.content ?? []; total.value = res.totalElements ?? 0; }
  catch { tableData.value = []; total.value = 0; }
  finally { loading.value = false; }
}

function openCreate() {
  editingId.value = null; dialogTitle.value = "新增用户";
  form.value = { username: "", email: "", password: "", role: "USER" };
  dialogVisible.value = true;
}

function openEdit(row) {
  editingId.value = row.id; dialogTitle.value = "编辑用户";
...
async function handleSubmit() {
  ...
  nameError.value = nameErr || ""; mailError.value = mailErr || "";
  if (nameErr || mailErr) return;
  if (!editingId.value) { const pwdErr = validatePassword(form.value.password); pwdError.value = pwdErr || ""; if (pwdErr) return; }
  submitting.value = true;
  try {
    if (editingId.value) { await updateUser(editingId.value, { username: form.value.username, email: form.value.email, role: form.value.role }); ElMessage.success("用户更新成功"); }
    else { await createUser(form.value); ElMessage.success("用户创建成功"); }
...
async function handleDelete(id, username) {
  try {
    ...
    await deleteUser(id); ElMessage.success("用户已删除"); await loadData();
  } catch { /* cancelled */ }
}
    ...
    <div class="page-card" style="flex:1; min-height:0">
      ...
      <div ref="wrapperRef" class="table-with-pagination">
        <el-table :data="tableData" v-loading="loading" style="width: 100%" :max-height="tableHeight" stripe>
          ...
          <el-table-column prop="role" label="角色" width="130"><template #default="{ row }"><el-tag :type="roleTagType[row.role] || 'info'" size="small">{{ roleLabel[row.role] || row.role }}</el-tag></template></el-table-column>
          <el-table-column prop="createdAt" label="创建时间" width="170"><template #default="{ row }">{{ (row.createdAt || "").replace("T", " ").slice(0, 16) }}</template></el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520" destroy-on-close>
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="邮箱" required :error="mailError">
          <el-input v-model="form.email" placeholder="请输入邮箱" @blur="onMailBlur(form.email)" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :disabled="!formValid" :loading="submitting" @click="handleSubmit()">{{ editingId ? "保存修改" : "确认创建" }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
