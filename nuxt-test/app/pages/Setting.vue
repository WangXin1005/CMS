...
const settings = ref<Record<string, string>>({ site_name: '', site_description: '', site_logo: '', icp_number: '' })
const settingKeys = [
...
async function loadSettings() {
  try {
    ...
    const list = res.data ?? []
    list.forEach((item: any) => { if (item.settingKey in settings.value) settings.value[item.settingKey] = item.settingValue ?? '' })
  } catch { /* ignore */ }
}

async function saveSetting(key: string) {
  try { await request.put(`/admin/settings/${key}`, { value: settings.value[key] }); ElMessage.success('保存成功') }
  catch { ElMessage.error('保存失败') }
}

async function saveAll() {
  loading.value = true
  try { await Promise.all(settingKeys.map(s => request.put(`/admin/settings/${s.key}`, { value: settings.value[s.key] }))); ElMessage.success('全部设置已保存') }
    ...
    <div class="page-card">
      <el-form label-width="120px" style="max-width:640px">
        <el-form-item v-for="s in settingKeys" :key="s.key" :label="s.label">
          <el-input v-if="s.type === 'text'" v-model="settings[s.key]" :placeholder="`请输入${s.label}`" />
          <el-input v-else v-model="settings[s.key]" type="textarea" :rows="3" :placeholder="`请输入${s.label}`" />
