<!-- logs — 操作日志页（ADMIN/SUPERADMIN）：分页查看、详情展开、变更对比 -->
...
// ElMessage/ElMessageBox 未使用，已移除
...
function resolveLabel(key: string, val: string | null): string | null {
  ...
  if (key === 'categoryId') {
    ...
    return catMap.value[Number(val)] || val
  }
  if (key === 'tagIds') {
    ...
    if (parsed.o || parsed.n) {
      ...
      for (const key of keys) {
        ...
        if (
          ...
          (displayNew.length > 10 || displayOld.length > 10)
        ) {
          diff = computeUnifiedDiff(displayOld, displayNew)
        }
        fields.push({
          label: fieldLabels[key] || key,
          oldVal: isHtml ? rawOld : displayOld,
          newVal: isHtml ? rawNew : displayNew,
          diff,
          isHtml,
        })
      }
        ...
        <el-table-column label="操作类型" width="160" align="center" header-align="center">
          <template #default="{ row }">
            <el-tag
              ...
              size="small"
              effect="dark"
              >{{ actionMap[row.action] || row.action }}</el-tag
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="操作类型">
            <el-tag
              ...
              size="small"
              effect="dark"
              >{{ actionMap[detailRow.action] || detailRow.action }}</el-tag
            >
          ...
          <el-descriptions-item label="时间" :span="2">{{
            (detailRow.createdAt || '').replace('T', ' ').slice(0, 16)
          }}</el-descriptions-item>
          ...
          <template v-if="parseDataFields(detailRow.details)">
            <template v-if="parseDataFields(detailRow.details)!.hasOld">
              <el-descriptions :column="1" border size="small">
                <el-descriptions-item
                  v-for="(field, i) in parseDataFields(detailRow.details)!.fields"
                  :key="i"
                  :label="field.label"
                >
                  <template v-if="field.diff">
                    <div class="diff-block">
                      <div v-if="!field.isHtml">
                        <span
                          v-for="(seg, si) in field.diff"
                          :key="si"
                          :class="'diff-' + seg.type + '-text'"
                          >{{ seg.text }}</span
                        >
                      </div>
                      <div v-else>
                  ...
                  <template v-else-if="field.oldVal !== null">
                    <div class="diff-block">
                      <div v-if="!field.isHtml">
                        <span class="diff-del-text">{{ field.oldVal }}</span>
                      </div>
                      <div v-else>
                        <span class="diff-del-text" v-html="sanitizeHtml(field.oldVal)"></span>
                      </div>
                      <div v-if="!field.isHtml">
                        <span class="diff-add-text">{{ field.newVal }}</span>
                      </div>
                      <div v-else>
                        <span class="diff-add-text" v-html="sanitizeHtml(field.newVal)"></span>
                      </div>
                    </div>
                  </template>
                  <template v-else>
                    <div v-if="!field.isHtml" class="diff-block">
                      <span class="diff-add-text">{{ field.newVal }}</span>
                    </div>
            ...
            <template v-else>
              <el-descriptions :column="1" border size="small">
                <el-descriptions-item
                  ...
                  :label="field.label"
                >
                  <span v-if="!field.isHtml" style="color: #333; word-break: break-all">{{
                    field.newVal
                  }}</span>
                  <span
                    v-else
                    style="color: #333; word-break: break-all"
                    v-html="sanitizeHtml(field.newVal)"
                  ></span>
                </el-descriptions-item>
              </el-descriptions>
            </template>
