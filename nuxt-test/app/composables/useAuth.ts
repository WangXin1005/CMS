/**
 * useAuth — 认证管理组合式函数
 * 封装登录/注册/初始化/密码修改等 API，管理 Token/用户名/角色 Cookie
 */
...
export const useAuth = () => {
   ...
   */
  const initSuperAdmin = async (data: { username: string; email: string; password: string }) => {
   ...
   * @param data.password - 密码
   */
  const registerGuest = async (data: { email: string; username: string; password: string }) => {
